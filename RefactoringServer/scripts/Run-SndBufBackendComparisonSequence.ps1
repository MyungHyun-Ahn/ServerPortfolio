param(
    ...
    [int]$RoomCount = 80,
    [int]$RoomCapacity = 4,
    [Nullable[int]]$RoomChangeProbabilityPercent = $null,
    ...
    [string[]]$ModeOrder = @("RioDirect", "RioOwnerThread", "IocpSendBuf0", "IocpSendBufDefault"),
    [string]$OutputLabel = "",
...
function Update-ClientConfigYaml
{
    ...
    $yaml = Set-YamlScalarValue -Content $yaml -SectionName "EchoClient" -Key "IntervalMs" -Value $IntervalMs
    ...
    switch ($ModeName)
    {
        "RioDirect"
        {
            return @{
                ...
                EffectiveSocketSendBufferBytes = 0
        ...
        "IocpSendBuf0"
        {
            return @{
                ...
                SocketSendBufferBytes = 0
                EffectiveSocketSendBufferBytes = 0
                DirectoryName = "iocp_sndbuf_0"
            }
        }
        "IocpSendBufDefault"
        {
            return @{
                Name = "IocpSendBufDefault"
                Backend = "Iocp"
                RioSendDispatchMode = "Direct"
                SocketSendBufferBytes = -1
                EffectiveSocketSendBufferBytes = -1
                DirectoryName = "iocp_sndbuf_default"
            }
        }
        default
        {
...
$effectiveRoomChangeProbabilityPercent = if ($null -ne $RoomChangeProbabilityPercent)
{
    $RoomChangeProbabilityPercent.Value
}
elseif ($IntervalMs -eq 0)
{
    10
}
else
...
@(
    ...
    "room_change_probability_percent=$effectiveRoomChangeProbabilityPercent",
...
foreach ($modeName in $ModeOrder)
{
    ...
    $serverYaml = Update-ServerConfigYaml `
        ...
        -SocketSendBufferBytes $mode.SocketSendBufferBytes `
    ...
    $clientYaml = Update-ClientConfigYaml `
        ...
        -RoomChangeProbabilityPercent $effectiveRoomChangeProbabilityPercent `
    ...
    $summaryRows.Add([pscustomobject]@{
        ...
        Backend = $mode.Backend
        RioSendDispatchMode = $mode.RioSendDispatchMode
        ConfigSocketSendBufferBytes = $mode.SocketSendBufferBytes
        EffectiveSocketSendBufferBytes = $mode.EffectiveSocketSendBufferBytes
        HoldSeconds = $HoldSeconds
