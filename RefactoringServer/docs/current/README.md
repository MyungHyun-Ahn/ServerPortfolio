...
Scope: Current entry documents

`docs/current`은 사람과 AI가 먼저 읽는 최신 기준 문서 모음이다.  
원문 히스토리는 `plans`, `reviews`에 남기고, 여기에는 지금 유효한 결론만 압축해서 둔다.
...
- 작업 우선순위: [000_plan-status-board.md](D:\Project\ServerPortfolio\RefactoringServer\docs\plans\000_plan-status-board.md)

## 3. 운영 규칙
- 새 설계 결론 또는 실험 결론이 나오면 해당 `current` 문서를 같이 갱신한다.
- 세부 근거와 긴 과정은 `plans`와 `reviews`로 링크한다.
- 더 이상 기준 문서가 아니면 `Canonical: No` 또는 후속 current 문서 링크로 교체한다.
