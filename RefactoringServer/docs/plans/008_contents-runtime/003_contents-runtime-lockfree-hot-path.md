...
- 경로를 `hot path`와 `non-hot path`로 나눠서 우선순위를 정한다.
- 이후 최적화는 “전체 lock-free”가 아니라 “효과가 큰 경로부터 단계적으로 경량화”하는 방향으로 간다.

## 2. 전제
- 현재 `ContentsRuntime`은 `std::mutex + std::condition_variable + std::deque` 기반이다.
- 구조 검증과 기능 검증은 이미 통과했고, `Login -> Chat snapshot -> Echo` 경로도 동작한다.
- 따라서 지금 단계의 질문은
  - 어디가 가장 자주 호출되는가
  - 어디가 contention을 만들 가능성이 큰가
...
- 대상 코드
  ...
  - packet 처리 루프
- 판단
  - producer는 여러 개, consumer는 하나라서 MPSC 패턴과 잘 맞는다.
  - packet inbox만 lock-free로 바꾸는 접근이 가장 보수적이다.
...
- 판단
  ...
  - 다만 `MoveSession`, disconnect와 정합성이 강하게 엮여 있으므로 완전 lock-free보다
    - slot array
    - shard
    - direct lookup
    같은 중간 단계가 더 현실적이다.
...
- 판단
  ...
  - 정합성이 우선이므로 지금은 hot path보다 한 단계 뒤로 둔다.
...
### 5.1 packet queue만 먼저 교체
- 가장 보수적인 접근
- `enter/leave`는 기존 구조 유지
- `packetQueue`만 lock-free MPSC queue로 교체

### 5.2 이벤트 통합은 후순위
- `Enter`, `Leave`, `Packet`을 하나의 이벤트 큐로 합치는 건 가능하다.
- 하지만 지금은 먼저 `packetQueue` 단독 효과를 보는 게 더 좋다.

### 5.3 session mapping은 즉시 완전 lock-free로 가지 않음
- read-heavy 경로이긴 하지만 정합성 리스크가 크다.
...
## 8. 결론
- 지금 단계에서 `ContentsRuntime` 전체 lock-free화는 과하다.
- 가장 유력한 hot path는 `packet inbox`이며, 여기부터 단계적으로 경량화하는 것이 안전하다.
