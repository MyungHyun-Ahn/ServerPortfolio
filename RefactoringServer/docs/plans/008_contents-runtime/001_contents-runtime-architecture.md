...
## 1. 목적
- 레거시 프로젝트의 `콘텐츠 전용 스레드 + 프레임 기반 처리` 패턴은 가져오되, `NetworkLib`와의 결합은 낮춘다.
- `NetworkLib`는 네트워크 I/O와 세션 수명주기에 집중하고, 콘텐츠 실행 모델은 별도 프로젝트 `ContentsRuntime`가 담당한다.

## 2. 경계 방향
- `ContentsRuntime`는 `NetworkLib` 바깥의 상위 계층이다.
- `NetworkLib`는 `sessionId + opcode + payload`까지만 전달한다.
- `ContentsRuntime`가 맡는 책임:
...
### 5.2 `IContentBridge`
- 콘텐츠가 `NetworkLib`를 직접 알지 않도록 하는 얇은 브리지
- 주요 함수
...
3. `ContentsRuntime`가 현재 세션 라우팅을 보고 대상 콘텐츠 스레드를 찾는다.
4. borrowed view 수명 문제를 피하기 위해 payload는 owned buffer로 변환한다.
5. 콘텐츠 스레드는 다음 프레임 루프에서 `OnPacket(...)`으로 처리한다.

## 7. 콘텐츠 이동 흐름
1. 예를 들어 `AuthContent`가 `LoginRq`를 처리한다.
2. 성공 시 `MoveSession(sessionId, kEchoContentId)`를 호출한다.
3. `ContentsRuntime`가 라우팅을 갱신한다.
4. 기존 콘텐츠에 `Leave`
5. 새 콘텐츠에 `Enter`
...
## 10. 후속 작업
- 콘텐츠 스레드 계측 강화
- packet inbox 경량화와 lock-free 검증
