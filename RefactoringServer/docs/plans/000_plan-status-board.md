...
| `007` | NetworkLib Performance | 진행 중 | 서버 OS 기준 장시간 성능 검증과 추가 벤치마크가 남아 있다. `IOCP + RIO` 병행 지원 구조는 1차 분리 작업을 완료했고, 순수 `RIO` baseline 구현도 끝냈다. 다음은 `RIO` 장시간 검증과 `IOCP` / `RIO` 비교다. |
