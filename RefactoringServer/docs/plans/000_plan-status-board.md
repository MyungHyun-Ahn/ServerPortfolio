...
- `추가 확인 필요`
  - 구현은 들어갔지만 장시간 검증, 정책 확정, 후속 확장이 남은 작업
...
| `002` | Legacy MhLib 조사/정리 | 완료 | 레거시 구조 비교와 참고 포인트 정리 완료. |
| `003` | NetworkLib Crypto / Packet Header | 완료 | cipher, framing, content header 기반 정리 완료. |
| `004` | NetworkLib Session | 완료 | 세션 수명주기와 참조 관리 구조 정리 완료. |
...
| `007` | NetworkLib Performance | 진행 중 | `IOCP + RIO` 병행 지원 구조 분리, pure `RIO` baseline, `Rio Direct / Rio OwnerThread / Iocp` 2시간 A/B 비교까지 완료. 현재 결론은 `Rio` 기본 send 정책은 `Direct` 유지가 적절하다는 것이다. 다음 단계는 `OwnerThread` 후속 최적화, `RIO` buffer 등록 비용 개선, 기본 `SendPacket` 경로 교체와 broadcast fan-out 기반 send copy 감소 구조 검토다. |
