...
| `007` | NetworkLib Performance | 진행 중 | 서버 OS 기준 장시간 성능 검증과 추가 벤치마크가 남아 있다. `IOCP + RIO` 병행 지원 구조는 1차 분리 작업을 완료했고, 다음은 `FRioServer` 실제 구현과 backend 비교다. |
