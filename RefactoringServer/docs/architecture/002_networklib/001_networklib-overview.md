...
## 1. 역할
- `NetworkLib`는 게임 로직을 직접 실행하는 모듈이 아니다.
- 비동기 네트워크 I/O, 세션 수명주기, 패킷 프레이밍/직렬화, 콘텐츠 패킷 전달 기반을 제공한다.
- 현재 목표는 `작게 검증 가능한 네트워크 코어`를 안정적으로 유지하는 것이다.

## 2. 현재 책임
- 소켓 생성, accept, recv/send, IOCP completion 처리
- 세션 생성/해제와 상태 관리
- transport header, content header, packet framing
- packet reader/writer, generated packet 직렬화/역직렬화
- zero-copy view(`string_view`, `bytes_view`)와 borrowed view guard
- 성능 계측, page pool/TLS pool 같은 최적화 보조 기능
...
- 현재 최상위는 다음 세 축으로 유지한다.
  - `Foundation`
  - `NetworkLib`
  - `Generated`
- `NetworkLib` 내부는 디렉터리 기준으로 나눈다.
...
- 콘텐츠 스레드, 콘텐츠 전이, 콘텐츠 프레임 루프는 `ContentsRuntime` 프로젝트가 담당한다.
