...
namespace MonitoringClientLib
{
	...
	{
		CSerializableBuffer<SERVER_TYPE::LAN> *reqLogin = Protocol::CGenPacket::makePacketReqMonitoringLogin(Setting::SERVER_NO);
	...
	// 모니터링 클라는 딱히 받을게 없음
	void CMonitoringClient::OnRecv(const UINT64 sessionID, CSerializableBuffer<SERVER_TYPE::LAN> *message) noexcept
	{
	}
}